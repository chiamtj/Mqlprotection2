<?php 
$GLOBALS["oetepzntakg"]="total_pages";
$GLOBALS["jpwwcyfeks"]="i";
$GLOBALS["alnwtioh"]="pagLink";
$GLOBALS["smbtzcwwc"]="result";
$GLOBALS["kqqtsoxkg"]="row";
$GLOBALS["wrugqltsbnvq"]="query";
$GLOBALS["mkbszmrmms"]="num_rows";
$GLOBALS["bopvyzitm"]="per_page";
$GLOBALS["cvltlam"]="page";
$GLOBALS["ycyfkvgywrr"]="conn";
$GLOBALS["kjyznwtmu"]="results";
$GLOBALS["kbxyixkvd"]="id";
$GLOBALS["dkiguihcpto"]="delete_q";
include"session.php";
...
<link rel=\"stylesheet\" href=\"validation/jquery.daterangepicker.min.css\">
<style>
...
<body>
<!--
<p style=\"background-color: #fa7878; color: #ffffff; font-family:Candara,Georgia,sans-serif;margin-bottom: auto;padding-left: 10px;\">  The developer who is working on the design can contact me on Telegram: @fx_seemore</p>
-->
<div class=\"header\">
    <p>
...
<div style=\"padding-top:0px;padding-left:10px;\">
    <ul>
        <li style=\"width:10px;height:34px;\">&nbsp;</li>
...
<div class=\"tab\">
    <button class=\"tablinks\" onclick=\"openTab(event, 'AddCustomer')\" id=\"";if(isset($_GET["customer"]))echo$_GET["customer"];echo "\">Add Customer</button>
    <button class=\"tablinks\" onclick=\"openTab(event, 'Password')\" id=\"";if(isset($_GET["pass"]))echo$_GET["pass"];echo "\">Change Password</button>
    <button class=\"tablinks\" onclick=\"openTab(event, 'Users')\" id=\"";if(isset($_GET["users"]))echo$_GET["users"];echo "\">Admin Users</button>
    <button class=\"tablinks\" onclick=\"openTab(event, 'Backup')\" id=\"";if(isset($_GET["backup"]))echo$_GET["backup"];echo "\">Backup/Restor</button>
</div>
...
<div id=\"Password\" class=\"tabcontent\">
    ...
    ";if(isset($_GET["perror"])){echo "      <p class=\"error\">";echo$_GET["perror"];echo "</p>
    ";}if(isset($_GET["psuccess"])){echo "      <p class=\"success\">";echo$_GET["psuccess"];echo "</p>
    ";}echo "
    <div class=\"main\">
...
<div id=\"Users\" class=\"tabcontent\">
    ...
    ";if(isset($_GET["uerror"])){echo "     <p class=\"error\">";echo$_GET["uerror"];echo "</p>
   ";}echo "   
    <div class=\"main\">
    ...
    ";if(isset($_POST["delete_admin"])){$id=$_POST["id"];$delete_q="delete from login where id = '".$id."'";if(mysqli_query($conn,$delete_q)){echo"<p class=\"success2\">Admin Deleted</p>";}else{echo"<p class=\"error2\">ERROR Admin Not Deleted!</p>";}}echo "   
    </div>

    <table class=\"table table-responsive table-hover\" style=\"font-family: sans-serif;\">
        <tbody>
      ";$results=mysqli_query($conn,"SELECT * FROM login");$num_rows=mysqli_num_rows($results);$per_page=10;$page=isset($_GET["pagea"])&&is_numeric($_GET["pagea"])?$_GET["pagea"]:1;$start_from=($page-1)*$per_page;$query="SELECT * FROM login LIMIT $start_from, $per_page";$result=mysqli_query($conn,$query);while($row=mysqli_fetch_array($result)){echo "       
          <tr >
                <form action=\"settings.php?users=defaultOpen\" method=\"POST\">
                    <td style=\"width:200px;\">
                    <input type=\"text\" name=\"admin_username\" placeholder=\"Admin Username\" class=\"input_field_text\" value=\"";echo$row["username"];echo "\" disabled>
                    </td>
                    <td><input type=\"hidden\" name=\"id\" value=\"";echo$row["id"];echo "\">
                    <input type=\"submit\" value=\"Del\" name=\"delete_admin\" id=\"delete_admin\" style=\"background-color:#ffa07a;cursor: pointer;\"></td>
                </form>
        </tr>
      ";}echo "  </tbody>
    </table>

    ";$pagLink="<ul class='pagination' style='margin-left:40px;'>";for($i=1;$i<=$total_pages;$i++){if(isset($search_text)){$pagLink.="<li class='page-item'><a class='page-link' href='settings.php?users=defaultOpen&pagea=".$i."&search_text=".$search_text."'>".$i."</a></li>";}else{$pagLink.="<li class='page-item'><a class='page-link' href='settings.php?users=defaultOpen&pagea=".$i."'>".$i."</a></li>";}}echo$pagLink."</ul>";echo "</div>
...
<div id=\"AddCustomer\" class=\"tabcontent\">
   ...
   ";if(isset($_GET["cerror"])){echo "     <p class=\"error\">";echo$_GET["cerror"];echo "</p>
   ";}if(isset($_GET["csuccess"])){echo "    <p class=\"success\">";echo$_GET["csuccess"];echo "</p>
   ";}echo "   
...
<div id=\"Backup\" class=\"tabcontent\">
    ...
    ";if(isset($_GET["berror"])){echo "    <p class=\"error2\">";echo$_GET["berror"];echo "</p>
  ";}if(isset($_GET["bsuccess"])){echo "    <p class=\"success2\">";echo$_GET["bsuccess"];echo "</p>
   ";}echo "
    <h3 style=\"margin-top:20px;font-size:22px;\">Backup/Restore DB</h3>

    <div class=\"row\">
        <form method=\"post\" action=\"set_restore_db.php\" enctype=\"multipart/form-data\" id=\"form-restore\">
    ...
    <form method=\"post\" action=\"set_import_customers.php\" enctype=\"multipart/form-data\" id=\"form-import\">
